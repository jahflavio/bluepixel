...
const filePath = path.join(__dirname, 'bluepixel-app', 'src', 'App.jsx');
              ...
              <div className="lg:w-1/3 flex flex-col gap-3">
                {cases.map((c, idx) => (
                  <button
                    ...
                    className={\`text-left p-6 rounded-2xl border transition-all duration-300 \${activeIndex === idx ? 'bg-navy-800 border-blue-500/30 shadow-[0_0_30px_-5px_rgba(59,130,246,0.15)]' : 'bg-transparent border-transparent hover:bg-white/[0.02] opacity-50 hover:opacity-100'}\`}
                  >
...
console.log("Successfully replaced CaseStudiesSection");
