...
function run() {
  ...
  // Hardcode the PERFECT CaseStudiesSection to avoid any extraction bugs
  const perfectCaseStudies = `const CaseStudiesSection = ({ onOpenCase }) => {
      const cases = [
        {
          id: 'bimbo',
          client: 'Grupo Bimbo',
  ...
  // 1. In baseContent, replace the old CaseStudiesSection with perfectCaseStudies
  ...
  // wait, the above `\n` in lastIndexOf needs to be actual newline
  oldCaseEnd = baseContent.lastIndexOf('\n', oldCaseEnd) + 1;
  if (oldCaseEnd <= oldCaseStart) {
      // fallback
      oldCaseEnd = baseContent.indexOf('const EngineeringLeadership = () => (');
  }

  baseContent = baseContent.substring(0, oldCaseStart) + perfectCaseStudies + '\n\n    ' + baseContent.substring(oldCaseEnd);

  // Write to index.html
  fs.writeFileSync(path.join(__dirname, 'index.html'), baseContent, 'utf8');
  console.log("Successfully wrote index.html");

  // 2. Extract JSX for App.jsx
  ...
  if (jsxStart > jsxStartStr.length - 1 && jsxEnd !== -1) {
    ...
    // Fix the `const { useState, useEffect, useRef } = React;` conflict
    jsxContent = jsxContent.replace('const { useState, useEffect, useRef } = React;', '');
