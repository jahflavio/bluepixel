...
// Insert FAQ component before EngineeringLeadership
txt = txt.replace('const EngineeringLeadership = () => (', faqCode);

// Replace ROICalculator in App with FAQSection
const oldTag = '<ROICalculator onOpenContact={(data) => scrollToForm()} />';
const newTag = '<FAQSection />';

if (txt.includes(oldTag)) {
    txt = txt.replace(oldTag, newTag);
    fs.writeFileSync('../index.html', txt, 'utf8');
    console.log("FAQ Injected successfully!");
} else {
    console.log("Could not find ROICalculator tag in App");
}
