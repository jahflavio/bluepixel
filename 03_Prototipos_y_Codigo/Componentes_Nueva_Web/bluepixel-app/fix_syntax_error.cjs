...
// Find the first instance of WorkflowTeardown
const compStart = txt.indexOf('const WorkflowTeardown = () => {');

// Find the next component AgenticTechStack
const nextComp = txt.indexOf('const AgenticTechStack = () => {');

// We know that between compStart and nextComp, there should ONLY be one WorkflowTeardown definition.
// The file is corrupted because it has the new WorkflowTeardown followed by the leftover return() from the previous one.
// Let's just extract the clean new WorkflowTeardown, and replace the whole block from compStart to nextComp with the clean one + the comment.
...
// The old block that needs to be replaced is everything from compStart to nextComp, minus the comment line
const commentIndex = txt.lastIndexOf('// Tech Stack & Ecosystem', nextComp);
const blockToReplace = txt.substring(compStart, commentIndex);

const newTxt = txt.replace(blockToReplace, cleanComponent.substring(0, cleanComponent.indexOf('// Tech Stack')));

fs.writeFileSync('../index.html', newTxt, 'utf8');
console.log("Syntax error in WorkflowTeardown fixed!");
